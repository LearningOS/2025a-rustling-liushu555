...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must guarantee that `address` is a valid, unique, mutable pointer to a `u32`.
    let ptr = address as *mut u32;
    // SAFETY: We trust the caller that `ptr` is valid and points to a mutable u32.
    *ptr = 0xAABBCCDD;
}
